...
void AudioAnalyzeFFT256MR_F32::update(void)
{
  ...
  // shift input4 array
  memmove(input4, &input4[AUDIO_BLOCK_SAMPLES / 4], sizeof(input4[0]) * (FFT_SIZE - AUDIO_BLOCK_SAMPLES / 4));

  // fill new samples into input4 array
  arm_fir_decimate_f32(&decimate4, block->data, &input4[FFT_SIZE - AUDIO_BLOCK_SAMPLES / 4], AUDIO_BLOCK_SAMPLES);

  // shift input16 array
  memmove(input16, &input16[AUDIO_BLOCK_SAMPLES / 16], sizeof(input16[0]) * (FFT_SIZE - AUDIO_BLOCK_SAMPLES / 16));

  // fill new samples into input16 array
  arm_fir_decimate_f32(&decimate16, &input4[FFT_SIZE - AUDIO_BLOCK_SAMPLES / 4], &input16[FFT_SIZE - AUDIO_BLOCK_SAMPLES / 16], AUDIO_BLOCK_SAMPLES / 4);
